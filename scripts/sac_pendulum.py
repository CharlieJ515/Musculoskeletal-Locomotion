...
def evaluate(
    ...
    *,
    ...
    fps = fps or env.metadata.get("render_fps", 30)
    tmpdir = get_tmp()
    returns = []
    ...
    for ep in range(episodes):
        ...
        ep_step = 0
        while not done:
            st = torch.from_numpy(s).float().unsqueeze(0).to(agent.device)
            a = agent.select_action(st)
            a_np = a.squeeze(0).cpu().numpy()
            s_next, r, terminated, truncated, _ = env.step(a_np)
        ...
        video_path = eval_dir / f"evaluation_step{step}_ep{ep}.mp4"
...
def main():
    ...
    for t in range(1, total_steps + 1):
        if t < start_random:
            ...
            with torch.no_grad():
                st = to_tensor(s).unsqueeze(0).to(device)
    ...
    # Save checkpoint
    ckpt_path = Path("sac_pendulum.pt")
