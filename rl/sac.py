...
class SAC(BaseRL):
    def __init__(
        ...
        self.actor = actor_net(self.state_dim, self.action_dim, self.log_std_min, self.log_std_max)
        
        # TODO - critics should receive reward_dim in initialization
        self.Q1 = critic_net(self.state_dim, self.action_dim)
        self.Q2 = critic_net(self.state_dim, self.action_dim)
        self.Q1_target = critic_net(self.state_dim, self.action_dim)
        self.Q2_target= critic_net(self.state_dim, self.action_dim)
        ...
        # Network Optimizers
        self.actor_optim = torch.optim.Adam(self.actor.parameters(), lr=self.lr, weight_decay=self.weight_decay)
        self.Q_optim = torch.optim.Adam(list(self.Q1.parameters()) + list(self.Q2.parameters()), lr=self.lr, weight_decay=self.weight_decay)
    ...
    def _update_critic(self, transition: TransitionBatch) -> Dict[str, float]:
        """
        ...
        q1_pred = self.Q1(s, a)
        q2_pred = self.Q2(s, a)
        critic_loss = 0.5 * (F.mse_loss(q1_pred, q_target) + F.mse_loss(q2_pred, q_target))
    ...
    def _update_actor(self, transition:TransitionBatch) -> Dict[str, float]:
        """
