...
def joint_index(name: str) -> int:
    try: return get_index_bundle().joint[name]
    except KeyError: raise KeyError(f"Unknown joint '{name}'")

def body_index(name: str) -> int:
    try: return get_index_bundle().body[name]
    except KeyError: raise KeyError(f"Unknown body '{name}'")

def muscle_index(name: str) -> int:
    try: return get_index_bundle().muscle[name]
    except KeyError: raise KeyError(f"Unknown muscle '{name}'")

def force_index(name: str) -> int:
    try: return get_index_bundle().force[name]
    except KeyError: raise KeyError(f"Unknown force '{name}'")
